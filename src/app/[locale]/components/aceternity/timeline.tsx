      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-24">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky mt-1.5 scr_2_2:-mt-[0.085rem] scr_4:-mt-[0.4rem] flex flex-col z-40 items-center top-40 self-start max-w-xs">
              <div className="h-10 absolute -left-2.5 w-10 rounded-full dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2" />
              </div>
              <h3 className="hidden text-5xl scr_2_0:text-4xl scr_2_2:text-3xl scr_4:text-xl font-bold text-muted-foreground ">
                {item.title}
              </h3>
            </div>

            <div className="relative grid pl-10 pr-4 w-full">
              <h3 className=" block text-4xl scr_2_2:text-3xl scr_4:text-xl mb-4 text-left font-bold text-muted-foreground">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-yellow-600 via-yellow-500 to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
